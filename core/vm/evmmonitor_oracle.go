...
type Oracle interface {
	...
	BeforeOperation(op OpCode, pc uint64, ctx *callCtx)
	AfterOperation(op OpCode, pc uint64, ctx *callCtx)
	Report() []Report
}
...
func (o *GaslessSendOracle) BeforeOperation(op OpCode, pc uint64, ctx *callCtx) {
	if op == CALL {
...
func (o *GaslessSendOracle) AfterOperation(op OpCode, pc uint64, ctx *callCtx) {
	return
}
